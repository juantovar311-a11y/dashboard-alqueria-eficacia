...
import io, json, zipfile, xml.etree.ElementTree as ET, re, os, threading, time
...
def clean(v):
    if v is None: return ''
    s = str(v).strip()
    return '' if s.upper() in {'0','N/A','NA','NULL','NONE','NAN'} else s
...
def normalize_rows(rows):
    ...
    for vals in rows[1:]:
        ...
        tipo=clean(pick(d,'TIPO EXHIBICIÓN','TIPO EXHIBICION','TIPO DE EXHIBICIÓN','TIPO DE EXHIBICION'))
        if not (cod or pdv or camp or tipo):continue
        c=norm(pick(d,'CUMPLE ESTRATEGIA','CUMPLIMIENTO'))
        cumple='SI' if c in {'SI','YES','TRUE','1'} else ('NO' if c in {'NO','FALSE','0'} else (c or 'SIN DATO'))
        out.append({
            'cod':cod,'pdv':pdv,'fecha':excel_date(pick(d,'FECHA')),'camp':camp,'tipo':tipo,'cumple':cumple,
            'obj':nval(pick(d,'CANTIDAD OBJETIVO','OBJETIVO')),
            'impl':nval(pick(d,'CANTIDAD IMPLEMENTADA ESTRATEGIA','CANTIDAD IMPLEMENTADA','IMPLEMENTADO')),
            'pct':nval(pick(d,'% IMPLEMENTACIÓN ESTRATEGIA','% IMPLEMENTACION ESTRATEGIA','% IMPLEMENTACIÓN','% IMPLEMENTACION')),
            'causal':clean(pick(d,'CAUSAL ESTRATEGIA','CAUSAL')),
            'gestion':clean(pick(d,'GESTIÓN ESTRATEGIA','GESTION ESTRATEGIA','GESTIÓN','GESTION')),
            'sup':clean(pick(d,'SUPERVISOR')),
            'lider':clean(pick(d,'LIDER DE EJECUCION','LÍDER DE EJECUCIÓN','LIDER','LÍDER')),
            'region':clean(pick(d,'REGION','REGIÓN','REGIONAL')),
            'zona':clean(pick(d,'ZONA','ZONA COMERCIAL','ZONA DE EJECUCION','ZONA DE EJECUCIÓN')),
            'ciudad':clean(pick(d,'CIUDAD')),
        })
    return out
...
def refresh_remote_if_needed(force=False):
    ...
    headers={'User-Agent':'Mozilla/5.0 Dashboard-Alqueria-Eficacia/1.0','Cache-Control':'no-cache'}
    ...
    req=Request(EXCEL_URL,headers=headers)
    try:
        with urlopen(req, timeout=30) as resp:
            data=resp.read()
            if not data.startswith(b'PK'):
                raise ValueError('La URL no devolvió un archivo .xlsx. Debe ser un enlace de descarga directa y sin inicio de sesión.')
            CACHE['remote_etag']=resp.headers.get('ETag','')
            CACHE['remote_last_modified']=resp.headers.get('Last-Modified','')
            # Only rebuild if bytes changed
            if force or CACHE['xlsx_bytes'] != data or CACHE['rows']==0:
                CACHE['xlsx_bytes']=data
                parse_to_payload(data, 'Excel cloud')
...
if __name__=='__main__':
    ...
    print(f'Puerto: {PORT}')
    if EXCEL_URL:
        print('Fuente: Excel cloud configurado en DASHBOARD_EXCEL_URL')
        print(f'Revisión cloud: cada {REMOTE_SECONDS} segundos')
    else:
        print(f'Fuente local: {LOCAL_EXCEL}')
    print(f'Abre: http://localhost:{PORT}')
    print('El dashboard consulta data.json automáticamente cada ~60 segundos.\n')
